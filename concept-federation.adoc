...
NOTE: 當 NetApp Console 處於私人模式時，無法使用聯邦功能。link:concept-modes.html["深入瞭解 NetApp Console 模式"^]。
...
.協定
* link:task-federation-adfs.html["Active Directory 聯合驗證服務 (AD FS)"]
* link:task-federation-saml.html["安全性斷言標記語言 (SAML) 身分提供者"]


.身分提供者
* link:task-federation-entra-id.html["微軟Entra ID"]
* link:task-federation-ping.html["Ping聯邦"]
...
[role="quick-margin-para"]
設定您的身分識別提供者以信任 NetApp，方法是建立新的應用程式並提供 ACS URL、實體 ID 或其他認證資訊等詳細資料。服務供應商資訊會因身分識別提供者而異，因此請參閱特定身分識別提供者的文件以取得詳細資料。您需要與 IdP 管理員合作才能完成此步驟。
...
.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-4.png["四"] 在 NetApp Console 中測試您的聯盟
[role="quick-margin-para"]
在啟用聯合連線之前對其進行測試。使用控制台中聯合頁面上的測試選項來驗證您的測試使用者是否可以成功進行身份驗證。如果測試成功，則可以啟用連線。

.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-5.png["五"] 在 Console 中啟用您的連線
[role="quick-margin-para"]
