...
[role="lead"]
如果您是 VMware 用戶，可以使用 OVA 檔案在 vCenter 中部署 Console 代理程式。OVA 下載連結或 URL 可透過 NetApp Console 取得。
...
NetApp 支援在 VCenter 環境中安裝 Console 代理程式。OVA 檔案包含一個預先設定的 VM 映像，您可以將其部署到 VMware 環境中。您可以直接從 NetApp Console 下載檔案或透過 URL 進行部署。該檔案包含 Console 代理程式軟體和自簽名憑證。
