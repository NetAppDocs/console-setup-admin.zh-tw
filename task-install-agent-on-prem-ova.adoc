...
summary: 如果您是 VMware 用戶，可以使用 OVA 檔案在您的 vCenter 中安裝 Console 代理程式。OVA 下載連結或 URL 可透過 NetApp Console 取得。 
---
= 使用 vCenter 在內部部署安裝 NetApp Console 代理程式
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
如果您是 VMware 用戶，可以使用 OVA 檔案在您的 vCenter 中部署 Console 代理程式。OVA 下載連結或 URL 可透過 NetApp Console 取得。


NOTE: 當您使用 vCenter 工具部署 Console 代理程式時，可以使用 VM Web 主控台執行維護工作。link:task-agent-vm-config.html["了解有關代理的 VM 控制台的更多資訊。"]
...
NOTE: 您無法使用安裝在本機的 Console 代理程式來管理 Google Cloud 中的資源。若要管理 Google Cloud 資源，請在 Google Cloud 中安裝代理程式。
...
.結果
您現在應該擁有具有所需權限的 IAM 使用者存取金鑰。安裝 Console 代理程式後，請從 Console 將這些憑證與 Console 代理程式建立關聯。
...
== 在您的 vCenter 環境中安裝 Console 代理程式

NetApp 支援在您的 vCenter 環境中安裝 Console 代理程式。OVA 檔案包含一個預先設定的 VM 映像，您可以將其部署到 VMware 環境中。您可以直接從 NetApp Console 下載檔案或透過 URL 進行部署。該檔案包含 Console 代理程式軟體和自簽名憑證。
...
=== 在您的 vCenter 環境中部署代理程式

登入您的 vCenter 環境以部署代理程式。

.步驟
. 如果您的環境需要，請將自簽名憑證上傳到受信任憑證清單中。安裝完成後，您需要替換此憑證。
. 從內容庫或本機系統部署 OVA。
+
|===
...
. 在 *Customize template* 視窗中、填寫以下欄位：
+
** *代理資訊*
+
*** 如果您選擇了明確代理、請輸入代理伺服器主機名稱或 IP 位址和連接埠號碼、以及使用者名稱和密碼。
*** 如果您選擇了透明代理，請上傳對應的憑證。
...
+
**** NetApp 建議取消選取此核取方塊，以便安裝程式包含代理程式組態檢查。組態檢查會驗證代理程式是否具有對所需端點的網路存取權。如果由於連線問題導致部署失敗，您可以從代理程式主機存取驗證報告和日誌。在某些情況下，如果您確信代理程式具有網路存取權，則可以選擇跳過此檢查。例如，如果您仍在使用 link:reference-networking-saas-console-previous.html["先前的端點"] 用於代理程式升級，則驗證會失敗並顯示錯誤。為避免這種情況，請勾選此核取方塊以在不進行驗證檢查的情況下進行安裝。link:reference-networking-saas-console-previous.html#update-endpoint-list["了解如何更新終端節點列表"]。
...
. 選擇“下一步”。
. 檢閱 *Ready to complete* 視窗中的詳細資料，然後選取 *Finish*。
+
