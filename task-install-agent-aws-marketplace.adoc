...
---
= 從 AWS Marketplace 部署 Console 代理程式
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
您可以直接從 AWS Marketplace 在 AWS 中部署 Console 代理程式。若要從 AWS Marketplace 部署 Console 代理程式，您需要設定網路、準備 AWS 權限、檢視執行個體要求，然後部署 Console 代理程式。

在 AWS 中部署代理程式會在您選擇的 VPC 中啟動一個執行 Linux 和 Console 代理軟體的 EC2 執行個體。

.開始之前
* link:concept-agent-install-options.html#agent-marketplace-deploy["了解何時從 AWS Marketplace 部署"]。
