...
summary: 當NetApp Console在 AWS 中啟動代理程式時，它會將一個原則附加到代理，該策略為代理提供管理該 AWS 帳戶內的資源和流程的權限。代理程式使用權限對多個 AWS 服務進行 API 呼叫，包括 EC2、S3、CloudFormation、IAM、金鑰管理服務等。 
---
= 用於 NetApp Console agent 的 AWS 權限
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
當NetApp Console在 AWS 中啟動控制台代理時，它會將一個原則附加到該代理，該原則會為代理提供管理該 AWS 帳戶內的資源和流程的權限。代理程式使用權限對多個 AWS 服務進行 API 呼叫，包括 EC2、S3、CloudFormation、IAM、金鑰管理服務 (KMS) 等。
...
.14+| 為Cloud Volumes ONTAP實例建立和管理 IAM 角色和實例設定檔 | iam:列出實例設定檔 | 是的 | 是的 | 不 
...
.3+| 使用 AWS 金鑰管理服務 (KMS) 啟用Cloud Volumes ONTAP的資料加密 | kms:重新加密* | 是的 | 不 | 不 
