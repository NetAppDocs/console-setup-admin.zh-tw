...
summary: 該控制台透過整合資料服務統一混合多雲的儲存管理和保護，以保護和最佳化資料。 
---
= 了解NetApp Console
:hardbreaks:
:allow-uri-read: 
...
NetApp持續為不需要出站連線的站點提供BlueXP 。BlueXP可以安裝在您的網路上，無需任何出站連線。 link:task-quick-start-private-mode.html["了解沒有網路連線的站點的BlueXP （私人模式）。"]
