...
您可以設定與 Microsoft Entra ID 的聯合連接，從而為 NetApp Console 啟用單一登入 (SSO)。此過程包括將您的 Microsoft Entra ID 設定為信任 NetApp Console 作為服務提供者，然後在 NetApp Console 中建立連線。
...
+
*** 對於 *Redirect URL*，請使用 `https://services.cloud.netapp.com`
...
. 建立與 Microsoft Entra ID 的連線。
+
.. 輸入您在上一個步驟中建立的用戶端 ID 和用戶端密碼。
