...
summary: NetApp Console提供 SaaS 體驗，可存取所有功能；也提供自架選項，適用於網路連線受限的組織。 
---
= 了解NetApp Console
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
NetApp Console提供 SaaS 體驗，可存取所有功能；也提供自架選項，適用於網路連線受限的組織。

您可以透過兩種方式使用NetApp Console ：作為完全託管的 SaaS 體驗，或作為您自己的雲端環境中的自架解決方案。使用 SaaS 選項，您可以透過 console.netapp.com 存取控制台，而NetApp管理所有基礎架構、驗證和更新。您可以透過網頁瀏覽器直接存取所有功能。如果選擇自架選項，則需要在 AWS、Azure 或 Google Cloud 環境中安裝控制台代理程式。此介面在本地運行，代理程式與NetApp保持有限的出站連接，以執行身份驗證和更新等基本任務。

如果您的組織擁有標準的網路連接，請選擇 SaaS 體驗。您可以享受最簡單的設定、自動更新以及對所有NetApp儲存系統、資料服務和進階功能的完全存取權。您可以直接從控制台新增代理， NetApp會處理所有維護工作。

如果您的組織需要嚴格的連接性、安全性或合規性控制，例如政府或受監管行業，請選擇自託管選項。您可以獲得更多控制權和網路隔離，但只能管理選定的NetApp儲存系統並使用有限的資料服務，且某些功能不可用。您可以手動安裝控制台代理，並直接從本機代理介面存取控制台。透過雲端市場或手動安裝安裝代理，代理僅與NetApp進行出站通信，確保沒有入站網路連線。
...
image:diagram-standard-mode.png["一張概念圖，展示了標準模式的架構，其中公共互聯網提供了 SaaS 層和 Auth0 身份驗證。"]
...
image:diagram-restricted-mode.png["受限模式的概念圖。"]
