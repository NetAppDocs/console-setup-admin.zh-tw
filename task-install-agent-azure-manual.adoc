...
---
= 在 Azure 中手動安裝 NetApp Console 代理程式
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
在 Azure 中部署代理程式會在您選擇的 VNet 中啟動執行 Linux 和 Console 代理程式軟體的 VM。


NOTE: 在與 Cloud Volumes ONTAP 系統相同的 Azure 區域中部署 Console 代理程式，或在 https://docs.microsoft.com/en-us/azure/availability-zones/cross-region-replication-azure#azure-cross-region-replication-pairings-for-all-geographies["Azure 區域對"^]中部署。這樣可以確保 Cloud Volumes ONTAP 與其相關儲存帳戶之間使用 Azure Private Link 連線。

.開始之前
* link:concept-agent-install-options.html#agent-cloud-manual-deploy["了解何時在 Azure 中手動部署"]。
