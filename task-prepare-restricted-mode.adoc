...
summary: 在受限模式下部署NetApp Console之前，請準備好您的環境。您需要查看主機需求、準備網路、設定權限等。 
---
= 準備在受限模式下部署
:hardbreaks:
:allow-uri-read: 
...
請使用已安裝的 NetApp Console 代理程式在本機提供的基於瀏覽器的介面。您無法透過 SaaS 應用程式提供的基於 Web 的主控台存取 NetApp Console。
...
準備網路以供使用者存取NetApp Console:: 在受限模式下，使用者從控制台代理 VM 存取控制台。控制台代理聯繫幾個端點來完成資料管理任務。當從控制台完成特定操作時，將從使用者的電腦聯繫這些端點。



NOTE: 4.0.0 版本之前的 Console 代理程式支援不同的端點。如果您已升級至 4.0.0 或更高版本，則可以從允許清單中移除舊的端點。link:reference-networking-saas-console-previous.html["了解有關 4.0.0 之前版本所需的網路存取的更多資訊。"]
...
|===
用於日常運營的出站互聯網訪問:: 控制台代理程式的網路位置必須具有出站網際網路存取權限。它需要能夠存取NetApp Console的 SaaS 服務以及各自公有雲環境中的端點。
+
--
[cols="2a,1a"]
|===
