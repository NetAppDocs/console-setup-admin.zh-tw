...
---
= 在 NetApp Console 中為合作夥伴使用者提供資源存取
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* link:reference-iam-keystone-roles.html["Keystone管理員"]
* link:reference-iam-keystone-roles.html["Keystone檢視器"]
...
. 選擇*合作關係*。
. 選擇*合作夥伴關係已啟動*選項卡。
. 選擇要查看的已建立合作關係旁的操作選單 image:icon-action.png["帶有三個垂直點的圖標"]，然後選擇 *View details*。
+
*成員*清單顯示合作夥伴組織已新增至合作夥伴關係的成員。

. 選擇要為其指派角色的成員旁邊的操作選單 image:icon-action.png["帶有三個垂直點的圖標"]，然後選擇 *新增角色*。
. 若要新增角色，請完成對話方塊中的步驟：
+
...
.. 若要取消指派成員的角色，請選取角色旁邊的 image:icon-delete.png["類似垃圾桶的圖標"]，即可從該成員移除對應的角色。系統會提示您確認移除操作。
