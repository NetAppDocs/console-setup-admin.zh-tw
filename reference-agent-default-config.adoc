...
summary: 了解 AWS、Azure 和 Google Cloud 上標準部署（可存取網際網路）的控制台代理預設配置，以及本機環境受限部署（無法存取網際網路）的控制台代理程式預設配置。 
---
= NetApp Console 中的 Console agent 預設組態
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
了解 AWS、Azure 和 Google Cloud 上標準部署（可存取網際網路）的控制台代理預設配置，以及本機環境受限部署（無法存取網際網路）的控制台代理程式預設配置。
...
* `/opt/application/netapp/service-manager-2/logs` （從新安裝的 3.9.23 版本開始）
+
...
* Console 代理服務名為 `occm`。
* 該  `occm` 服務依賴 MySQL 服務。
+
如果 MySQL 服務當機，那麼  `occm` 服務也會當機。
