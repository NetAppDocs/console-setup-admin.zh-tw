...
summary: 代理程式的 Google Cloud 防火牆規則需要同時設定傳入和傳出規則。NetApp Console 在您從 Console 建立 Console 代理程式時，會自動建立此安全性群組。對於其他安裝選項，您需要手動設定此安全性群組。 
---
= NetApp Console 代理程式在 Google Cloud 中的防火牆規則
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
代理程式的 Google Cloud 防火牆規則需要同時設定傳入和傳出規則。NetApp Console 在您從 Console 建立 Console 代理程式時，會自動建立此安全性群組。對於其他安裝選項，您需要手動設定此安全性群組。
...
| DNS | UDP | 53 | DNS | 用於 Data Classification 的 DNS 解析 
|===