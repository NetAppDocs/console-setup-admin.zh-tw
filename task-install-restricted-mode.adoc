...
+
link:task-prepare-restricted-mode.html#step-3-review-host-requirements["審查代理要求"]。
...
+
託管識別允許控制台代理虛擬機器無需憑證即可向 Microsoft Entra ID 進行身份驗證。 https://docs.microsoft.com/en-us/azure/active-directory/managed-identities-azure-resources/overview["詳細了解 Azure 資源的託管標識"^] 。
...
.Azure 服務主體
--
請為 NetApp Console 提供您先前設定的 Azure 服務主體認證。
...
.結果
NetApp Console 現在已擁有在 Azure 代表您執行操作所需的權限。
