...
[role="lead"]
您可以在 NetApp Console 中管理您的聯盟。您可以停用它、更新過期的認證，或在不再需要時將其刪除。
...
[NOTE]
====
* 如果您使用NetApp Cloud Central 設定了聯合，請透過 *Federation* 頁面匯入它以在控制台中進行管理。link:task-federation-import.html["了解如何匯入您的聯盟"]
* 您可以在「審核」頁面上查看聯盟管理事件，例如啟用、停用和更新聯盟。 link:task-monitor-cm-operations.html["了解有關在NetApp Console中監控操作的更多資訊。"]
...
. 選擇*管理>身分和存取*。
. 選擇“*Federation*”選項卡。
. 選擇要測試的聯盟旁邊的動作功能表 image:button_3_vert_dots.png["帶有三個垂直點的圖標"]，然後選擇 *測試連線* 。
...
. 選擇*管理>身分和存取*。
. 選擇“*Federation*”選項卡。
. 選擇要停用的聯盟旁邊的動作功能表 image:button_3_vert_dots.png["帶有三個垂直點的圖標"]，然後選擇 *停用* 。
...
. 選擇要刪除的聯盟旁邊的動作功能表 image:button_3_vert_dots.png["帶有三個垂直點的圖標"]，然後選擇 *Delete* 。
