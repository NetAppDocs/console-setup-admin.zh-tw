...
| link:reference-iam-platform-roles.html#organization-admin-roles["組織管理員"] | 允許使用者無限制地存取組織內的所有專案和資料夾。可以為任何專案或資料夾新增成員，執行任何任務，並使用任何未明確關聯角色的資料服務。擁有此角色的使用者可以透過建立資料夾和專案、分配角色、新增使用者以及管理系統（如果他們擁有相應的憑證）來管理您的組織。這是唯一可以建立 NetApp Console 代理的存取角色。 
...
| link:reference-iam-platform-roles.html#super-admin-roles["超觀眾"] | 提供使用者一部分檢視者角色。此角色專為規模較小的組織所設計，這些組織可能不需要將 NetApp Console 職責分配給多個使用者。 
|===
...
| link:reference-iam-gcnv-roles.html["Google Cloud NetApp Volumes檢視器"] | 具有Google Cloud NetApp Volumes使用者角色的使用者可以查看Google Cloud NetApp Volumes。 
...
| link:reference-iam-backup-rec-roles.html["備份和還原復原管理員"] | 在 Backup and Recovery 中還原工作負載。 


| link:reference-iam-backup-rec-roles.html["備份和還原克隆管理員"] | 在 Backup and Recovery 中複製應用程式和資料。 
...
| link:reference-iam-data-classification.html["分類檢視器"] | 允許使用者查看 NetApp Data Classification 掃描結果。擁有此角色的使用者可以查看合規性資訊，並為其有權存取的資源產生報告。這些使用者無法啟用或停用磁碟區、儲存桶或資料庫架構的掃描。 
