...
| *compute.forwardingRules.create* | 建立轉送規則，將流量路由到後端服務。 | 不 | 是的 | 不 


| *compute.forwardingRules.delete* | 刪除現有轉送規則。 | 不 | 是的 | 不 


| *compute.forwardingRules.get* | 取得現有轉送規則的詳細資訊。 | 不 | 是的 | 不 


| *compute.forwardingRules.setLabels* | 設定或更新組織轉送規則的標籤。 | 不 | 是的 | 不 


| compute.globalOperations.get | 獲取操作狀態。 | 是的 | 是的 | 不 


| *compute.healthChecks.create* .4+| 建立和管理健康檢查，以監控後端服務的運作狀況。 | 不 | 是的 | 不 
...
| *計算實例使用* | 使用虛擬機器實例（啟動、停止、連線操作）。 | 不 | 是的 | 不 
...
| *compute.resourcePolicies.create* .3+| 建立和管理資源策略，實現資源自動化管理。 | 不 | 是的 | 不 


| *compute.resourcePolicies.delete* | 不 | 是的 | 不 
...
| *logging.logEntries.create* .2+| 建立並路由日誌條目，用於監控、偵錯和稽核。 | 是的 | 是的 | 不 


| *logging.logEntries.route* | 是的 | 是的 | 不 
...
| *compute.addresses.createInternal* | 在VPC網路內建立內部IP位址以進行資源分配。 | 不 | 是的 | 不 


| *compute.addresses.deleteInternal* | 刪除內部 IP 位址以進行資源清理。 | 不 | 是的 | 不 


| *compute.addresses.setLabels* | 更新地址資源上的標籤。 | 不 | 是的 | 不 


| *compute.addresses.useInternal* | 網路通訊請使用內部IP位址。 | 不 | 是的 | 不 
