...
---
= 在 AWS 中為 NetApp Console 手動部署 Console 代理程式
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
您可以在 AWS 中執行的 Linux 主機上手動部署 Console 代理程式。若要在您自己的 Linux 主機上手動部署 Console 代理程式，您需要檢閱主機需求、設定網路、準備 AWS 權限、部署 Console 代理程式，然後提供您準備好的權限。

在 AWS 中部署代理程式會在您選擇的 VPC 中啟動一個執行 Linux 和 Console 代理軟體的 EC2 執行個體。

.開始之前
* link:concept-agent-install-options.html#agent-cloud-manual-deploy["了解何時在 AWS 中手動部署"]。
...
使用 IMDSv2 時的 PUT 響應跳數限制:: 如果啟用了 IMDSv2（新 EC2 執行個體的預設設定），請將 PUT 回應跳數限制設為 3。否則，系統會在代理設定期間顯示 UI 初始化錯誤。
+
--
...
.. 完成剩餘步驟以建立策略。
+
根據您計劃使用的 NetApp 資料服務、您可能需要建立第二個原則。對於標準區域、權限分佈在兩個原則中。由於 AWS 中託管原則的字元大小上限、因此需要兩個原則。link:reference-permissions-aws.html["了解有關控制台代理的 IAM 策略的更多信息"]。
...
如果您在建立 Console 代理程式的同一個 AWS 帳戶中有 Amazon S3 儲存桶，您將在 *Systems* 頁面上自動看到 Amazon S3 儲存系統。 https://docs.netapp.com/us-en/storage-management-s3-storage/index.html["了解如何從NetApp Console管理 S3 儲存桶"^]
