...
image:diagram_azure_authentication.png["概念圖，顯示控制台在進行 API 呼叫之前，如何從 Microsoft Entra ID 取得驗證和授權。控制台角色定義了 Active Directory 中的權限。它與一個或多個 Azure 訂閱以及代表控制台應用程式的服務主體物件相關聯。"]
...
.結果
您可以從「詳細資料和認證」頁面切換到不同的認證集 https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/task-deploying-otc-azure.html["將系統新增至控制台時"^]。
