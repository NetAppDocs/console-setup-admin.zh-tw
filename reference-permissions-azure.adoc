...
summary: 當NetApp控制台在 Azure 中啟動控制台代理程式時，它會將自訂角色附加到 VM，該 VM 為代理程式提供管理該 Azure 訂閱中的資源和流程的權限。代理程式使用權限對多個 Azure 服務進行 API 呼叫。 
---
= 控制台代理程式的 Azure 權限
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
當NetApp控制台在 Azure 中啟動控制台代理程式時，它會將自訂角色附加到 VM，該 VM 為代理程式提供管理該 Azure 訂閱中的資源和流程的權限。代理程式使用權限對多個 Azure 服務進行 API 呼叫。
...
+
** link:task-install-connector-azure-marketplace.html#step-3-set-up-permissions["設定 Azure 市場部署的權限"]
...
當您使用NetApp資料分類掃描Azure NetApp Files資料時，代理程式會發出以下 API 請求：
...
控制台代理程式對NetApp備份和復原發出以下 API 請求：
...
| Microsoft.Network/networkInterfaces/寫入 | 是的 | 不 
...
| Microsoft.Network/networkInterfaces/寫入 | 是的 | 是的 | 不 
...
| Microsoft.Resources/訂閱/資源群組/讀取 | 不 | 是的 | 不 
...
當您設定NetApp Cloud Tiering 時，代理會發出以下 API 請求。
...
將磁碟區資料備份到 Azure Blob 儲存體時， NetApp Backup and Recovery 不再需要以下權限：
