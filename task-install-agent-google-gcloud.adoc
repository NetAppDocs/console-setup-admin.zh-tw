...
---
= 從 Google Cloud 部署 Console 代理程式
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
若要使用 Google Cloud 在 Google Cloud 中部署 Console 代理程式，您需要設定網路、準備 Google Cloud 權限、啟用 Google Cloud API，然後部署 Console 代理程式。

在 Google Cloud 中部署代理程式會在您選擇的 VPC 中啟動一個執行 Linux 和 NetApp Console 代理軟體的 VM 執行個體。

.開始之前
* 您應該有一個 link:concept-agents.html["了解控制台代理"]。
...
開啟 Web 瀏覽器並前往 https://console.netapp.com["NetApp Console"^]開始使用控制台代理。
