...
---
= 從 NetApp Console 在 Azure 部署 Console agent
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
若要從 NetApp Console 在 Azure 中部署 Console 代理程式，您需要設定網路、準備 Azure 權限，然後部署 Console 代理程式。
...
.步驟
. 選擇“*管理 > 代理*”。
. 在 *Overview* 頁面上，選擇 *Deploy agent > Azure*。
. 在*審核*頁面上，審核部署代理程式的要求。這些要求也在本頁上方詳細說明。
. 在 *Virtual Machine Authentication* 頁面上，選擇與您設定 Azure 權限的方式相符的驗證選項：
+
...
. 在「詳細資料」頁面上，輸入代理程式的名稱、指定標籤，並選擇是否要讓 Console 建立具有所需權限的新角色，或是要選取您使用 link:reference-permissions-azure.html["所需的權限"] 設定的現有角色。
+
...
如果您在建立控制台代理程式的相同 Azure 帳戶中擁有 Azure Blob 存儲，您將看到 Azure Blob 儲存體會自動出現在 *系統* 頁面上。 https://docs.netapp.com/us-en/bluexp-blob-storage/index.html["了解如何透過NetApp Console管理 Azure Blob 儲存"^]
