...
summary: 瞭解 NetApp Console 管理的新功能。 
---
= NetApp Console 有哪些新功能
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
.更新了本機 UI 存取的 SSL 憑證
在受限模式或私有模式下使用 BlueXP 時，使用者介面可透過部署在雲端區域或內部部署的 Connector 虛擬機器存取。預設情況下，BlueXP 使用自我簽署的 SSL 憑證，透過本機網頁瀏覽器提供安全的 HTTPS 存取。
...
我們更新了 Connector 的 AWS 和 Azure 原則，移除了不再需要的權限。這些權限與 BlueXP 邊緣快取以及在 Azure 和 AWS 中探索和管理 Kubernetes 叢集有關。
...
----
這些權限是 Cloud Volumes ONTAP 支援虛擬機器規模集所必需的。如果您已有連接器並希望使用此新功能，則需要將這些權限新增至與 Azure 憑證關聯的自訂原則。
...
如果您現有的代理程式執行在 RHEL 7 或 CentOS 7 上，NetApp 則不支援將作業系統升級或轉換為 RHEL 8 或 9。您需要在受支援的作業系統上建立一個新的代理程式。請依照以下簡要步驟進行切換：
...
** https://docs.netapp.com/us-en/storage-management-fsx-ontap/use/task-create-fsx-system.html["建立或發現 FSx for ONTAP系統"^]
