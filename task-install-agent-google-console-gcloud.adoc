...
summary: 若要從 NetApp Console 或使用 Google Cloud 建立 Console agent，您需要設定網路、準備 Google Cloud 權限、啟用 Google Cloud API，然後建立 Console agent。 
---
= 從 NetApp Console 在 Google Cloud 部署 Console agent
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
您可以從 Console 在 Google Cloud 中部署 Console 代理程式。您需要設定網路、準備 Google Cloud 權限、啟用 Google Cloud API，然後部署 Console 代理程式。

在 Google Cloud 中部署代理程式會在您選擇的 VPC 中啟動一個執行 Linux 和 NetApp Console 代理軟體的 VM 執行個體。

.開始之前
* link:concept-agent-install-options.html#agent-console-deploy["瞭解何時從 NetApp Console 部署"]。
...
從NetApp控制台聯繫的端點:: 當您使用基於 Web 的 NetApp Console SaaS 應用程式時，它會連接多個端點以完成資料管理任務。這包括用於從 Console 部署 Console 代理的端點。
+
--
link:reference-networking-saas-console.html["從 NetApp Console 檢視已聯絡的端點清單"]。
...
NOTE: 在 Google Cloud 中部署代理程式時，代理程式會建立一個儲存桶來儲存部署文件。
...
.步驟
. 選擇“*管理 > 代理*”。
. 在 *Overview* 頁面上，選擇 *Deploy agent > Google Cloud*。
...
** *網路標籤*：如果使用透明 Proxy，請在 Console 代理程式執行個體中新增網路標籤。網路標籤必須以小寫字母開頭，並且可以包含小寫字母、數字和連字號。標籤必須以小寫字母或數字結尾。例如，您可以使用標籤  `console-agent-proxy`。
** *防火牆原則*：選擇是要建立新的防火牆原則，還是選擇允許所需傳入和傳出規則的現有防火牆原則。
+
...
如果您在建立 Console 代理程式的同一個 Google Cloud 帳戶中擁有 Google Cloud Storage 儲存桶，則 Google Cloud Storage 系統會自動顯示在 *Systems* 頁面上 https://docs.netapp.com/us-en/storage-management-google-cloud-storage/index.html["了解如何透過控制台管理 Google 雲端存儲"^]。
