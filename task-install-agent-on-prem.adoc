...
NOTE: 如果您是 VMware 使用者，您可以使用 OVA 在您的 vCenter 中部署 Console 代理程式。link:task-install-agent-on-prem-ova.html["深入瞭解如何在 vCenter 中部署代理程式。"]

安裝前，您需要確保您的主機（虛擬機器或 Linux 主機）符合要求，並確保 Console 代理程式可以存取網際網路以及目標網路。如果您打算使用 NetApp 資料服務或雲端儲存選項（例如 Cloud Volumes ONTAP），則需要在雲端提供者建立憑證並將其新增至 Console，以便 Console 代理程式可以代表您在雲端執行操作。
...
請在符合作業系統、RAM 和連接埠要求的 x86 主機上執行 Console 代理程式。安裝 Console 代理程式之前，請確保您的主機符合這些要求。
...
如果您想在 AWS 或 Azure 中使用 NetApp 資料服務與內部部署 Console 代理程式，則需要在雲端供應商中設定權限，然後在安裝 Console 代理程式後將認證新增至 Console 代理程式。
...
手動安裝 Console 代理程式時，需要準備滿足要求的機器環境。您需要一台 Linux 機器，並根據您的 Linux 作業系統安裝 Podman 或 Docker。
...
--
.開始之前
如果您剛剛建立了這些 Azure 認證資料，可能需要幾分鐘才能使用。請稍候幾分鐘，再將認證資料新增至 Console 代理程式。
