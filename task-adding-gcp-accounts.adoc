...
---
= 將控制台代理程式憑證與您的 Google Cloud 訂閱關聯
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
在 Google Cloud 中建立控制台代理程式時，您可以使用相關憑證訂閱NetApp Intelligent Services（包括Cloud Volumes ONTAP）和NetApp資料服務。您必須將訂閱與您想要使用其憑證來支付服務的代理商關聯起來。
...
用新的市場訂閱取代目前的市場訂閱會更新現有和新的Cloud Volumes ONTAP系統的訂閱。
...
透過 Google Cloud Marketplace 訂閱NetApp資料服務可能會因權限不正確或錯過重定向到控制台而失敗。如果發生這種情況，請按照以下步驟完成訂閱程序。
