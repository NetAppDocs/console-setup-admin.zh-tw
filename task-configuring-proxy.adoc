...
---
= 設定 NetApp Console 代理程式以使用 Proxy 伺服器
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
IMPORTANT: 設定 Proxy 後，您無法變更 Proxy 類型。如果您需要變更 Proxy 類型，則必須移除 Console 代理程式並新增具有新 Proxy 類型的新代理程式。
...
您可以使用下列方法更新控制台代理現有的透明代理伺服器： `proxy update`透過使用命令來移除透明代理伺服器 `proxy remove`命令。更多信息，請查閱相關文件。link:reference-agent-maint-console-proxy.html["代理維護控制台"] 。
...
如果您已將 Console 代理程式設定為使用 Proxy 伺服器，則可以在 Console 代理程式上啟用直接 API 流量，以便將 API 呼叫直接傳送至雲端供應商服務，而無需透過 Proxy。在 AWS、Azure 或 Google Cloud 中執行的代理程式均支援此選項。
