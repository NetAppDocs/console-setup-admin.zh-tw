...
---
= 在 Google Cloud 中手動部署 NetApp Console 代理程式
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
若要在您自己的 Linux 主機上手動部署 Console 代理程式，您需要檢閱主機需求、設定網路、準備 Google Cloud 權限、啟用 Google Cloud API、安裝 Console，然後提供您準備好的權限。

在 Google Cloud 中部署代理程式會在您選擇的 VPC 中啟動一個執行 Linux 和 NetApp Console 代理軟體的 VM 執行個體。

.開始之前
* link:concept-agent-install-options.html#agent-cloud-manual-deploy["了解何時在 Google Cloud 中手動部署"]。
...
. 如果將此代理程式與 Cloud Volumes ONTAP 系統關聯，請考慮增加與該代理程式關聯的執行緒數量以提高效能。link:https://kb.netapp.com/Cloud/ncds/nds/cvods/cvods_kbs/CVO_going_to_failed_state_frequently_in_NetApp_Console["了解如何增加代理程式的 Container 執行緒限制"^]。
...
連接埠:: 除非您發起請求或將其用作代理以將 AutoSupport 訊息從 Cloud Volumes ONTAP 傳送到 NetApp 支援部門，否則 Console 代理不會接收任何傳入流量。
+
--
...
. 如果您打算在 Console 代理程式所在的專案以外的其他專案中部署 Cloud Volumes ONTAP 系統、則需要為 Console 代理程式的服務帳戶提供這些專案的存取權限。
+
...
+
** NetApp Console：前往 *Agents ＞ Management ＞ Deploy agent ＞ On-prem ＞ Manual install*。
+
選擇下載 agent 安裝程式檔案或檔案的 URL。

** https://mysupport.netapp.com/site/products/all/details/netapp-console/downloads-tab["NetApp支援站點"^]（如果您還沒有存取 NetApp Console 的權限，則需要此步驟）。
+

NOTE: 下載適用於網路或雲端的「線上」代理安裝程式。NetApp Console 代理還有一個單獨的「離線」安裝程式，但僅支援私有模式部署。

** NetApp 支援網站（如果您還沒有存取 Console 的權限，則需要此網站） https://mysupport.netapp.com/site/products/all/details/netapp-console/downloads-tab["NetApp支援站點"^]。
...
NOTE: 如果您想要設定透明代理伺服器，可以在安裝完成後進行。link:reference-agent-maint-console-proxy.html["了解代理維護控制台"]。

+
以下是使用 CA 簽署憑證設定明確 Proxy 伺服器的範例：
...
如果您在建立控制台代理程式的相同 Google Cloud 帳戶中擁有 Google Cloud Storage 儲存桶，您將看到 Google Cloud Storage 系統自動出現在 *Systems* 頁面上。 https://docs.netapp.com/us-en/storage-management-google-cloud-storage/index.html["了解如何透過NetApp Console管理 Google Cloud Storage"^]
