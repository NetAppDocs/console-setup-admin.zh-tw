...
* 安裝python3
* 安裝對應的 podman-compose 包
* 將 podman-compose 加入到 PATH 環境變量
* 請確認您的 Podman 版本使用的是 Netavark Aardvark DNS 而不是 CNI。
...
. 安裝 Podman。
+
從 Red Hat Enterprise Linux 軟體倉庫取得 Podman。
...
+
請依照以下步驟安裝支援的 Docker Engine 版本。請勿安裝最新版本，因為主機不支援該版本。
...
NOTE: 如果要設定透明代理，可以在安裝完成後進行設定。 link:reference-agent-maint-console.html["了解代理維護控制台"]
