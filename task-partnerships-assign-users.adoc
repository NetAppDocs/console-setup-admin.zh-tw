...
.必備角色
建立和管理合作夥伴關係需要 *Partnership admin* 角色。*Partnership viewer* 可以檢視 Partnerships 頁面。link:reference-iam-predefined-roles.html["了解有關訪問角色的更多資訊。"]

您可以隨時將使用者從合作夥伴關係中移除。從合作夥伴關係中移除使用者會立即撤銷其對合作夥伴組織中任何資源的存取權。
...
當您在合作夥伴關係中新增成員時，合作夥伴組織的 *Partnership admin* 必須先為他們指派組織內特定資源的角色，然後他們才能存取這些資源。
...
. 選擇*合作關係*。
. 選擇“*已收到合作關係*”標籤。
. 選擇要新增成員的已建立合作關係旁邊的動作功能表 image:icon-action.png["帶有三個垂直點的圖標"]，然後選擇 *新增成員*。
. 選擇一個或多個成員新增至合作夥伴關係，然後選取 *Add* 。
...
. 選擇*合作關係*。
. 選擇“*已收到合作關係*”標籤。
. 選擇要移除的成員旁邊的動作功能表 image:icon-action.png["帶有三個垂直點的圖標"]，然後選擇 *移除關聯*。
. 在對話方塊中選擇“*刪除*”來確認該操作。
