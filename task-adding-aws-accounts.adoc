...
[role="lead"]
新增和管理 AWS 憑證，以便您可以從 NetApp Console 部署和管理 AWS 帳戶中的雲端資源。如果您管理多個 AWS Marketplace 訂閱，則可以在「憑證」頁面中將每個訂閱指派給不同的 AWS 憑證。
...
link:concept-accounts-aws.html["了解NetApp Console如何使用 AWS 憑證和權限"]。
...
NOTE: 如果您從控制台部署了控制台代理，它會自動為您部署控制台代理的帳戶新增 AWS 憑證。這確保了管理資源所需的必要權限。
...
如果您想為 NetApp Console 提供 IAM 使用者的 AWS 金鑰，則需要授予該使用者所需的權限。NetApp Console IAM 原則定義了 NetApp Console 可以使用的 AWS 操作和資源。
...
設定 IAM 角色，使 NetApp Console 軟體即服務（SaaS）應用程式能夠承擔該角色。
...
----
+
如需 IAM 中跨帳戶資源存取的詳細資訊，請參閱 link:https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies-cross-account-resource-access.html["AWS Identity and Access Management (IAM) 說明文件"^]。
...
.結果
IAM 角色現在具有所需的權限。<<add-the-credentials,現在您可以將其新增至控制台>>。
...
.開始之前
如果您剛剛建立了 IAM 角色，可能需要幾分鐘才能生效。請稍等片刻，然後再將憑證新增至 NetApp Console。
...
. 在*組織憑證*頁面上，選擇*新增憑證*並按照精靈中的步驟進行操作。
+
.. *憑證位置*：選擇*Amazon Web Services > 控制台*。
...
如需詳細資訊，請參閱 https://docs.netapp.com/us-en/storage-management-fsx-ontap/requirements/task-setting-up-permissions-fsx.html["Amazon FSx for ONTAP 的 Console 說明文件"^]
...
.開始之前
您必須先建立 NetApp Console 代理程式才能設定訂閱。link:concept-agent-install-options.html["了解如何建立控制台代理"]。
...
從 AWS Marketplace 訂閱時，最後一步是將訂閱與您的組織建立關聯。如果您未完成此步驟，則無法將該訂閱與您的組織一起使用。
...
.步驟
. 選擇“*管理 > 憑證*”。
. 在*組織憑證*頁面上，選擇一組憑證的操作選單，然後選擇*編輯憑證*。
. 進行所需的更改，然後選擇*應用*。
