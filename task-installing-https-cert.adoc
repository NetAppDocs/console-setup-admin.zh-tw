...
[role="lead"]
在受限模式或私有模式下使用 NetApp Console 時，使用者介面可透過部署在雲端區域或內部部署的 Console 代理虛擬機器存取。預設情況下，Console 使用自簽名 SSL 憑證，為在 Console 代理上執行的 Web 型主控台提供安全的 HTTPS 存取。
